...
namespace Client.Main.Controls.Terrain
{
    ...
    public class TerrainRenderer
    {
        ...
        private readonly DynamicLightGpuUploader _dynamicLightUploader = new();
        ...
        private void UploadDynamicLights(Effect effect)
        {
            ...
            int maxLights =
                Constants.OPTIMIZE_FOR_INTEGRATED_GPU
                    ? 8
                    : 32;
            ...
            Vector2 focusPos =
                Camera.Instance != null
                    ? new Vector2(
                        Camera.Instance.Target.X,
                        Camera.Instance.Target.Y)
                    : Vector2.Zero;

            LastUploadedDynamicLights =
        ...
        private void RenderTerrainTile(int xi, int yi, float lodFactor, int lodInt, bool after, byte edgeMask = 0)
        {
            ...
            bool canUseIndexedTile =
                _useTerrainIndexBatching &&
        ...
        private void PrepareTileVertices(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, int i1, int i2, int i3, int i4)
        {
        ...
        private void SetTempVertexFromSample(int slot, int worldTileX, int worldTileY, int sampleIndex)
        {
            ...
            else
            {
                ...
                if (_data.HeightMap != null &&
                    (uint)sampleIndex < (uint)_data.HeightMap.Length)
                {
