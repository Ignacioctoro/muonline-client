...
namespace Client.Main.Graphics
{
    ...
    public sealed class DynamicLightGpuUploader
    {
            ...
            float focusRadius = 0f)
        {
            ...
            int selectedCount = SelectRelevantLights(
                lights,
                focus,
            ...
            for (int i = 0; i < selectedCount; i++)
            {
                ...
                _lightPosInvRadius[i] =
                    new Vector4(
                        light.Position,
                        1f / radius);

                _lightColorIntensity[i] =
                    new Vector4(
                        light.Color,
                        intensity);
            }
        ...
        public static int ResolveEffectCapacity(
            Effect effect,
            int fallbackCapacity)
        {
            ...
            int pos =
                GetEffectArrayCapacity(
                    effect.Parameters["LightPosInvRadius"],
                    fallback);

            int color =
                GetEffectArrayCapacity(
                    effect.Parameters["LightColorIntensity"],
                    fallback);
        ...
        private static int GetEffectArrayCapacity(
            ...
            int budget)
        {
                 ...
                 i++)
            {
                ...
                {
                    _selectedIndices[weakestIndex] =
                        i;

                    _selectedScores[weakestIndex] =
                        score;
            ...
            SortSelectedByScoreDesc(
                selected);

            return selected;
        }

        private void SortSelectedByScoreDesc(
            int count)
        {
            for (int i = 1;
                 i < count;
