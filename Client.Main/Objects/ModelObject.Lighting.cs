...
namespace Client.Main.Objects
{
    ...
    {   
        ...
        private bool TryUploadGpuSkinBoneMatrices(Effect effect,int requiredBoneCount)
        {
            if (effect == null ||
                requiredBoneCount <= 0 ||
                requiredBoneCount > MaxGpuSkinBones)
            {
                return false;
            }

            Matrix[] bones =
                GetCachedBoneTransforms();

            bones =
                GetRenderBoneTransforms(bones)
                ?? bones;
            ...
            if (_gpuSkinBoneUploadBuffer == null ||
                _gpuSkinBoneUploadBuffer.Length != copyCount)
            {
                _gpuSkinBoneUploadBuffer =
                    new Matrix[copyCount];
            }
            ...
            effect.Parameters["BoneMatrices"]
                ?.SetValue(
                    _gpuSkinBoneUploadBuffer);
        ...
        private void PrepareDynamicLightingEffect(Effect effect, bool useGpuSkinning = false, int requiredBoneCount = 0)
        {
            ...
            bool usingSkinnedTechnique =
                skinnedTechnique != null &&
                TryUploadGpuSkinBoneMatrices(
                    effect,
                    requiredBoneCount);
            ...
            effect.Parameters["SunDirection"]
                ?.SetValue(sunDir);

            effect.Parameters["SunColor"]
                ?.SetValue(_sunColor);

            effect.Parameters["SunStrength"]
                ?.SetValue(
                    sunEnabled
                        ? SunCycleManager
                            .GetEffectiveSunStrength()
                        : 0f);
            ...
            effect.Parameters["AmbientLight"]
                ?.SetValue(
                    _ambientLightVector *
                    SunCycleManager.AmbientMultiplier);

            effect.Parameters["DebugLightingAreas"]
                ?.SetValue(
                    Constants.DEBUG_LIGHTING_AREAS
                        ? 1f
                        : 0f);

            // ---------------------------------------------------------
            // Terrain light under object
            // ---------------------------------------------------------
            ...
            effect.Parameters["TerrainLight"]
                ?.SetValue(terrainLight);
            ...
            int maxLights =
                ResolveDynamicObjectLightBudget(
                    worldTranslation);
            ...
            float focusRadius =
                ResolveDynamicObjectLightFocusRadius();
        ...
        private int ResolveDynamicObjectLightBudget(
            Vector3 worldTranslation)
        {
            bool isMonster =
                this is MonsterObject;

            int maxLights =
                isMonster
                    ? (
                        Constants
                            .OPTIMIZE_FOR_INTEGRATED_GPU
                            ? 2
                            : 4
                    )
                    : (
